		...
		if (isPipeReaderJobRunning.compareAndSet(false, true)) 
		{
			try 
			{
				...
				while ((packet = streamPacketQueue.poll()) != null) {
					...
					updateQualityParameters(dts, TIME_BASE_FOR_MS,10,isKeyFrame);
	...
	public void updateQualityParameters(long pts, AVRational timebase, long packetSize,boolean isKeyFrame) {
		...
		if(isKeyFrame) {
			if(lastKeyFramePts != 0) {
				...
				long timeDiff = (long) ( (keyFrameDiff * ( (double)timebase.num() / (double) timebase.den()) ) * 1000);
				if(keyFrameDiff>300)
					LoggerUtils.logJsonString("KeyFrameStats", "streamId", streamId , "keyFrameDiffMs", String.valueOf(timeDiff),"totalKeyFrame",Long.toString(keyFramePerMin));
			}
			lastKeyFramePts = pts;
			keyFramePerMin += 1;
		}
		...
		double bitrateFlushDuration = (long)  (timeDiff * ( (double)timebase.num() / (double) timebase.den())* 1000 );
 		...
 		if(bitrateFlushDuration > 1000){
			if (broadcastStream != null) {
				broadcast.setBitrate((totalByteReceived - broadcast.getReceivedBytes())/128);
				broadcast.setReceivedBytes(totalByteReceived);
			} else {
				broadcast.setBitrate(bitrate); // Bytes/s to kbps
				broadcast.setReceivedBytes(totalByteReceived);
			}
